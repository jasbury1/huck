...
/// A minimal binary min-heap ordered by a caller-supplied comparator — the priority
/// queue backing the fetch frontier. Insert and remove are O(log n); the standard
/// library has no heap, and the frontier can grow large on wide threads, so a plain
/// sorted array would be needlessly quadratic.
